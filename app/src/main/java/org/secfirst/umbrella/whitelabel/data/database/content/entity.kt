...
fun Content.toSearchResult(): SearchResult {
    ...
    return SearchResult("${WordUtils.capitalizeFully(segments[1])} - ${WordUtils.capitalizeFully(segments[3])}", this.check
        ...
        c.startActivity(Intent(Intent.ACTION_VIEW, Uri.parse("umbrella://checklists/${this.checklist?.id}")))
    }
}
...
fun createFeedSources(): List<FeedSource> {
    ...
    val feedSource2 = FeedSource("UK Foreign Office Country Warnings", false, 1)
