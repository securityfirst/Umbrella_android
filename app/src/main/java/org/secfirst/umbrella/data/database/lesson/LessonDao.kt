...
interface LessonDao {
    ...
    suspend fun getAllLesson(): List<Module> = withContext(ioContext) {
        ...
        modules.map {
            it.toModule().apply {
                this.subjects = subjects.filter { it.module_id == this.id }.map { it.toSubject() }
                    .toMutableList()
    ...
    suspend fun getDifficultyBySubject(subjectId: String): List<Difficulty> =
    ...
    suspend fun getDifficultyPreferred(subjectId: String): DifficultyPreferred? =
        withContext(ioContext) {
            SQLite.select()
