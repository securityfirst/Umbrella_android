...
class AppPreferenceHelper @Inject constructor(
    ...
    override fun setMockView(result: Boolean) =
        prefs.edit().putBoolean(EXTRA_SHOW_MOCK_VIEW, result).commit()

    override fun isShowMockView(): Boolean = prefs.getBoolean(EXTRA_SHOW_MOCK_VIEW, false)

    override fun setLanguage(isoCountry: String) =
        prefs.edit().putString(EXTRA_LANGUAGE, isoCountry).commit()
    ...
    override fun enablePasswordBanner(enableBanner: Boolean) =
        prefs.edit().putBoolean(STATE_PW_BANNER, enableBanner).commit()
    ...
    override fun setMaskApp(isMaskApp: Boolean) =
        prefs.edit().putBoolean(EXTRA_MASK_APP, isMaskApp).commit()

    override fun isLoggedIn() = prefs.getBoolean(EXTRA_LOGGED_IN, false)

    override fun setLoggedIn(isLoggedIn: Boolean) =
        prefs.edit().putBoolean(EXTRA_LOGGED_IN, isLoggedIn).commit()

    override fun getDarkMode(): Boolean = prefs.getBoolean(IS_DARK_MODE, false)

    override fun setDarkMode(state: Boolean) = prefs.edit().putBoolean(IS_DARK_MODE, state).commit()

    override fun setSkipPassword(isSkip: Boolean) =
        prefs.edit().putBoolean(EXTRA_SKIP_PASSWORD, isSkip).commit()

    override fun getSkipPassword() = prefs.getBoolean(EXTRA_SKIP_PASSWORD, false)

    override fun setRefreshInterval(position: Int) =
        prefs.edit().putInt(EXTRA_REFRESH_INTERVAL, position).commit()

    override fun getRefreshInterval() = prefs.getInt(EXTRA_REFRESH_INTERVAL, 30)

    override fun setSkipPathways(skip: Boolean) =
        prefs.edit().putBoolean(SKIP_PATHWAYS, skip).commit()
...
interface PreferenceHelper {
    ...
    fun setDarkMode(state: Boolean): Boolean
