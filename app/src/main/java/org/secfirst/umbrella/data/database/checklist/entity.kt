...
class Dashboard(var items: List<Item> = listOf()) {
    ...
    data class Item(
        ...
        constructor(
            progress: Int, label: String,
            checklist: Checklist?,
            difficulty: Difficulty?,
            levelLabel: Int = 0
        ) : this("", 0, progress, label, levelLabel, checklist, difficulty)
    }
}

fun Checklist.covertToHTML(): String {
    var body =
        "<html><head><meta Content-Type: text/html; charset=\"UTF-8\"></head><body style=\"font-family: Roboto; font-size:16px; font-weight: normal;\" >"
